...
SkedTape.defaultFormatters = {
	...
	date: function(date, endian, delim) {
		if(!delim){	delim='.';}
		switch(endian = endian || 'usa'){
			case "point": delim = ".";break;
			case "date": case "": case "default": delim = "/";break;
			case "less": delim = "-";break;
			default: delim = delim;break;
		}

		var nums = [date.getFullYear(), date.getMonth() + 1,date.getDate()];
		if (endian === 'usa') {nums = [nums[2] ,nums[1], nums[0]];}
...
SkedTape.prototype = {
	...
	addEvent: function(entry, opts) {
		if (!this.locationExists(entry.location)) {
			console.log('Unknown location #' + entry.location);
	...
	renderHours: function() {
		var $ul = $('<ul/>');
		var interval = this.interval //間隔刻度
		...
		while (tick.getTime() <= this.end.getTime()) {
			...
			tick.setTime(tick.getTime() + interval *60*1000);
		}
		...
		$.each(this.getLocations(), $.proxy(function(i, location) {
			...
			events.forEach(function(event) {
				...
				if (belongs && visible) {
					...
					// $event
					// 	.append('<div class="scorll scroll-right">');
	...
	renderEvent: function(event) {
		...
		if (event.url && !event.disabled) {
			$event = $('<a/>').attr('href', event.url);
		} else {
			$event = $('<div />');
			// $event = $('<div /><div class="scorll scroll-right" />');
		}
		...
		if (this.showEventTime || this.showEventDuration) {
			...
			if (this.showEventTime) {
				...
				html +=' [ 共'+ this.format.duration(duration)+' ] ';
			}
			if (this.showEventDuration) {
				html += '<br> '+this.showEventDurationText+' '+ event.memo;
	...
	computeEventOffset: function(event) {
		...
		let result = hoursBeforeEvent /  getDurationHours(this.start, this.end) * 100 + '%';
		return result;
	},
